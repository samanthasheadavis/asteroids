...
For this homework we were given the HTML and CSS files as well as some foundational JavaScript.

What I did:

* Created a ship object that stores ship's velocity and angle.
* Implemented functionality to handle when arrow keys are pressed:
...
* Created a game loop that manipulates the `top` and `left` style attributes of the ship object using JavaScript.
* Check if a collision with an asteroid has occurred using `getBoundingClientRect()`.
* End the game if a collision has occurred.
